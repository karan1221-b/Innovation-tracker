...
export default function Profile() {
  ...
  const MENU = [
    ...
    { icon: "shield-checkmark-outline", label: "Privacy & data", onPress: () => toast("Manage your data anytime — contact support", "info") },
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: insets.top + spacing.lg, paddingBottom: 100 }}>
        <View style={styles.head}>
          ...
          <Pressable testID="edit-profile-button" style={styles.editBtn} onPress={() => editSheet.current?.expand()}>
        ...
        <View style={styles.menu}>
          {MENU.map((m, i) => (
            <Pressable key={i} testID={`menu-${i}`} style={[styles.menuRow, i < MENU.length - 1 && styles.menuBorder]} onPress={m.onPress}>
              <Ionicons name={m.icon as any} size={20} color={colors.onSurfaceSecondary} />
      ...
      <BottomSheet ref={editSheet} index={-1} snapPoints={[380]} enablePanDownToClose keyboardBehavior="interactive" backdropComponent={(p) => <BottomSheetBackdrop {...p} appearsOnIndex={0} disappearsOnIndex={-1} />}>
        <BottomSheetView style={{ padding: spacing.xl }}>
          <Text style={styles.sheetTitle}>Edit profile</Text>
          <Text style={styles.fieldLabel}>Name</Text>
          <BottomSheetTextInput testID="edit-name-input" value={name} onChangeText={setName} style={styles.input} placeholder="Your name" placeholderTextColor={colors.muted} />
...
const styles = StyleSheet.create({
  ...
  head: { alignItems: "center", paddingHorizontal: spacing.xl },
  avatar: { width: 88, height: 88, borderRadius: 44, backgroundColor: colors.brandPrimary, alignItems: "center", justifyContent: "center" },
  avatarText: { fontSize: 36, fontWeight: "700", color: "#fff" },
  name: { fontSize: font.xl, fontWeight: "700", color: colors.onSurface, marginTop: spacing.md },
  email: { fontSize: font.base, color: colors.onSurfaceTertiary, marginTop: 2 },
  editBtn: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: spacing.md, paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: radius.pill, backgroundColor: colors.brandTertiary },
  editText: { color: colors.brandPrimary, fontWeight: "600", fontSize: font.base },
  menu: { backgroundColor: colors.surface, borderRadius: radius.lg, marginHorizontal: spacing.lg, marginTop: spacing.xl, borderWidth: 1, borderColor: colors.border, overflow: "hidden" },
  menuRow: { flexDirection: "row", alignItems: "center", gap: spacing.md, padding: spacing.lg },
  menuBorder: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.divider },
  menuLabel: { flex: 1, fontSize: font.base, color: colors.onSurface, fontWeight: "500" },
